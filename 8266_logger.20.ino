  ...
  function init() {
    ...
    dataPlot = new Chart(document.getElementById("line-chart"), {
      ...
      options: {
        title: {
          display: true,
          text: 'Logger 20 - Click Seconds, Minutes, ... and Click Temperature, Pressure, ... to add or remove it'
        },
        scales: {
           xAxes: [{
                type: 'time',
    ...
    webSocket.onclose = async (event) => {
        console.error(event);    
        dataPlot.options.title.text = " Logger20 - Hit Refresh - Socket has closed " ; 
   }
...
void setup() {
  ...
  Serial.begin(115200);
  Serial.println("             ");
  Serial.println("Logger20");

  WiFiManager wifiManager;
  //wifiManager.resetSettings();
  wifiManager.autoConnect("Logger20");
  Serial.println("connected...yeey :)");

  WiFi.mode(WIFI_STA);
  WiFi.hostname("Logger20");
