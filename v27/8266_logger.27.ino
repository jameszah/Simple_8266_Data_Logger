/*------------------------------------------------------------------------------
  ...
  v22 - Jan 11, 2021 - add file manager to download the data
  v23 - Jan 12, 2021 - save the 5min file everyday
  v24 - Jan 13, 2021 - new zip code, plus save 5m archive forever
  v25 - Jan 14, 2021 - 
  v26 - Jan 15, 2021 - x axis labels
  v27 - Jan 17, 2021 - move donwload button
...
<br>
  ...
  <button onClick="f_fman()" id="fman">Download Data from Flash</button>
...
<script>
  ...
  function init() {
    ...
    dataPlot = new Chart(document.getElementById("line-chart"), {
      ...
      options: {
        title: {
          display: true,
          text: 'Logger v27 - Click Seconds, Minutes, ... and Click Temperature, Pressure, ... to add or remove it'
        },
        scales: {
           xAxes: [{
                type: 'time',
    ...
    webSocket.onclose = async (event) => {
        console.error(event);    
        dataPlot.options.title.text = " Loggerv27 - Hit Refresh - Socket has closed " ; 
   }
    ...
    webSocket.onmessage = function(event) {
      ...
      if (mytype == "live") {
        ...
        dataPlot.options.scales.xAxes[0].time.unit  = "second";
         ...
         dataPlot.update();
          ...
          for(var i = 0; i <= len ; i++ ) {
           ...
           for(var i = 0; i < mydataarray.length; i++ ) {   
            ...
            if (mytype == "minutes") {
              dataPlot.options.scales.xAxes[0].time.unit  = "minute";
            }
            if (mytype == "5minutes") {
              dataPlot.options.scales.xAxes[0].time.unit  = "hour";
            }
            if (mytype == "hours") {
              dataPlot.options.scales.xAxes[0].time.unit  = "day";
            }
            if (mytype == "days") {
              dataPlot.options.scales.xAxes[0].time.unit  = "day";
            }
  ...
  function f_fman(){
    console.log('got data click');
    //window.open('http://127.0.0.1:8080');
    //console.log(webSocket.remoteAddress);
    console.log(webSocket.url);
    var theurl = webSocket.url;
    var newurl = 'http://' + theurl.substring(5,17) + ':8080';
    console.log(newurl);
    window.open(newurl);
  }
...
void Save_5Min(){
  ...
  strftime(fname, sizeof(fname), "/5minutes%Y%m%d.txt", tmstruct);
  
  Serial.println("Writing 5minutes for archive ... ");
  
  file = LittleFS.open("/5mArchive.txt", "a");
  if (!file) {
